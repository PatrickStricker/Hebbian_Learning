...
class UnitScale:
    ...
    def __call__(self, x):
        x_flat = x.reshape(-1)
        return x / (x_flat.norm(p=2) + self.eps)


class PerSampleRMSNormalize:
    def __init__(self, eps=1e-8):
        self.eps = eps

    def __call__(self, x):
        x_flat = x.reshape(-1)
        x_flat = x_flat - x_flat.mean()
        rms = torch.sqrt(torch.mean(x_flat ** 2) + self.eps)
        x_flat = x_flat / rms
...
class FeatureWhitenTorch:
    ...
    def fit(self, dataloader):
        ...
        for batch in dataloader:
            ...
            # same preprocessing used before estimating feature-wise statistics
            batch = batch - batch.mean(dim=1, keepdim=True)
            rms = torch.sqrt(torch.mean(batch ** 2, dim=1, keepdim=True) + 1e-8)
            batch = batch / rms
    ...
    def transform(self, x):
        ...
        return x.reshape(x.shape[0], *original_shape)


class FeatureWhitenTransform:
    def __init__(self, whitener):
...
def compose_optional(*transforms_):
    items = []
    for tr in transforms_:
        if tr is None:
            continue
        if isinstance(tr, transforms.Compose):
            items.extend(tr.transforms)
        else:
            items.append(tr)

    return transforms.Compose(items) if items else None
...
def make_data_loaders(dataset_config, batch_size, device, dataset_path=DATASET):
    ...
    dataset_train_class, dataset_class, test_transform, train_aug_transform, device, train_indices, val_indices, split, dataset_path = select_dataset(
        dataset_config, device, dataset_path
    )

    train_sampler = SubsetRandomSampler(train_indices, generator=g)

    use_author_preproc = dataset_config.get("author_preprocessing", False)

    # Build train dataset first without final preprocessing.
    # This prevents FeatureWhitenTorch.fit() from seeing already-whitened data.
    origin_dataset = dataset_train_class(
        dataset_path,
        split=split,
        train=True,
        download=not dataset_config['name'] in ['ImageNet'],
        transform=None,
        zca=dataset_config['zca_whitened'] and not use_author_preproc,
        device=device,
        train_class=dataset_config['training_class']
    )

    if use_author_preproc:
        print("Authors preprocessing pipeline")

        width = dataset_config["width"]
        height = dataset_config["height"]

        # Deterministic spatial preprocessing for fitting the feature whitener.
        # This is necessary especially for STL-10 if raw data are 96x96 but model input is 32x32.
        resize_transform = transforms.Resize((height, width), antialias=True)

        # Fit whitener on resized, non-augmented train data.
        # For publication experiments, this is the clean setting: no test leakage.
        origin_dataset.transform = resize_transform
        ...
        whitener = FeatureWhitenTorch(eps=1e-5)
        whitener.fit(fit_loader)

        hpca_preproc = transforms.Compose([
            PerSampleRMSNormalize(),
            FeatureWhitenTransform(whitener),
            UnitScale(),
        ])

        # Train: augmentation first, then RMS -> feature whiten -> unit L2.
        # If train_aug_transform is crop_flip(width, height), it already produces the target size.
        # If no augmentation is used, resize first.
        if dataset_config['augmentation']:
            final_train_transform = compose_optional(
                train_aug_transform,
                hpca_preproc,
            )
        else:
        ...
        # Test/validation: deterministic resize, then same fitted whitening, then unit scale.
        final_test_transform = compose_optional(
            resize_transform,
            hpca_preproc,
        )

        origin_dataset.transform = final_train_transform

    else:
        origin_dataset.transform = train_aug_transform
    ...
    if val_indices is not None:
        ...
        test_dataset = dataset_class(
            dataset_path,
            split="val" if dataset_config['name'] in [
                'ImageNet',
                'ImageNette',
                'ImageNetV2MatchedFrequency'
            ] else "test",
            train=False,
            zca=dataset_config['zca_whitened'] and not use_author_preproc,
        ...
        test_loader = torch.utils.data.DataLoader(
            test_dataset,
            batch_size=batch_size if dataset_config['name'] in [
                'STL10',
                'ImageNet',
                'ImageNette',
                'ImageNetV2MatchedFrequency',
                'ImageNetV2TopImages',
                'ImageNetV2Threshold07'
            ] else 1000,
            num_workers=dataset_config['num_workers'],
