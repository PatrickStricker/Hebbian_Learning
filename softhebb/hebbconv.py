...
#New Class to enable HPCA mode

class HebbPCAConv2d(HebbHardConv2d):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        padding_mode: str = "zeros",
        bias: bool = False,
        hpca_rule: str = "sanger",
        use_wta_for_hpca: bool = False,
        nb_train: int = None,
    ) -> None:
        """
        Convolutional Hebbian PCA / Oja / Sanger layer.

        full:
            dW = E[x y^T] - W E[y y^T]

        sanger:
            dW = E[x y^T] - W tril(E[y y^T])

        In implementation form, weights are flattened as [O, D], so the
        competitive term is computed as:

            update = E[y x] - E[y y^T] @ W
        """
        super(HebbPCAConv2d, self).__init__(
        ...
        self.hpca_rule = hpca_rule.lower()
        if self.hpca_rule not in ("full", "sanger"):
            raise ValueError(
                "hpca_rule must be either 'full' or 'sanger', "
                f"got {hpca_rule!r}"
            )

        self.use_wta_for_hpca = bool(use_wta_for_hpca)

    def get_pre_activations(self, x: torch.Tensor, mask: bool = False) -> torch.Tensor:
        """
        HPCA uses the ordinary linear convolutional projection y = W*x.
        Do not use the SoftHebb/Krotov p-norm transformed weight here.
        """
        return self._conv_forward(x, self.weight, self.bias)

    def compute_wta_mask(self, y: torch.Tensor) -> torch.Tensor:
        """
        Hard channel-wise WTA mask over output channels at every batch/spatial
        position. Shape is the same as y: [B, O, H, W].
        """
        winner = torch.argmax(y, dim=1, keepdim=True)
        return torch.zeros_like(y).scatter_(1, winner, 1.0)

    def get_wta(self, pre_x: torch.Tensor, group_id: int = 0) -> torch.Tensor:
        """
        For HPCA, the effective activity is the linear output itself. We still
        record a hard-WTA histogram for diagnostics only, matching the existing
        stat_wta/radius logging convention.
        """
        mask = self.compute_wta_mask(pre_x)
        self.stat[
            2,
            group_id * self.out_channels_groups: (group_id + 1) * self.out_channels_groups,
        ] += mask.sum((0, 2, 3)).cpu()
        return pre_x

    def compute_yx(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        Compute sum over batch and spatial positions of y times input patches.
        Return shape: [O, C, Kh, Kw].
        """
        yx = F.conv2d(
            x.transpose(0, 1),
            y.transpose(0, 1),
        ...
        return yx[:, :, :self.kernel_size[0], :self.kernel_size[1]]

    def update_hpca(self, x, y, weight):
        """
        Convolutional neural PCA / Oja-style update.
        ...
        In weight layout [O, D], this is implemented as:

            dW = E[y x^T] - E[y y^T] W

        The final max-abs normalisation is required because the original
        SoftHebb update loop expects bounded delta_weight tensors.
        """
        y_eff = y

        if self.use_wta_for_hpca:
            y_eff = y_eff * self.compute_wta_mask(y_eff)

        out_channels = y_eff.shape[1]

        hebb = self.compute_yx(x, y_eff).reshape(out_channels, -1)

        y_flat = y_eff.permute(1, 0, 2, 3).reshape(out_channels, -1)

        n_pos = max(y_flat.shape[1], 1)

        hebb = hebb / n_pos
        c_yy = (y_flat @ y_flat.t()) / n_pos

        if self.hpca_rule == "sanger":
            c_yy = torch.tril(c_yy)

        w_flat = weight.view(out_channels, -1)

        update = hebb - (c_yy @ w_flat)

        # Required for compatibility with the original SoftHebb update loop.
        nc = torch.abs(update).amax()
        update = update / (nc + 1e-30)

        return update.view_as(weight)
    ...
    def delta_weight(
        ...
        """
        Existing plasticity() passes the third argument as `wta`.
        For this class get_wta() returns the linear activity y, so this is HPCA.
        """
        return self.update_hpca(x, wta, weight)

    def extra_repr(self):
        s = ", hpca_rule=%s, use_wta_for_hpca=%s" % (
            self.hpca_rule,
            self.use_wta_for_hpca,
        )
        return self.extra_repr2() + s
    ...
    def __label__(self):
        ...
        rule = "S" if self.hpca_rule == "sanger" else "F"
        wta = "WTA" if self.use_wta_for_hpca else ""
        return "HPCA" + s.format(**self.__dict__) + rule + wta


def select_Conv2d_layer(params) -> Union[
    HebbHardConv2d,
    HebbHardKrotovConv2d,
    HebbSoftConv2d,
    HebbSoftKrotovConv2d,
    HebbPCAConv2d,
]:
    """
    ...
    # New HPCA Branch    
        ...
        layer = HebbPCAConv2d(
            ...
            padding_mode=params["padding_mode"],
            bias=params["add_bias"],
            hpca_rule=params.get("hpca_rule", "sanger"),
            use_wta_for_hpca=params.get("use_wta_for_hpca", False),
            nb_train=params["nb_train"],
        )
    else:
        raise ValueError("Unknown convolutional Hebbian softness: %s" % params["softness"])
    return layer
