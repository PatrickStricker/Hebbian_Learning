...
class FeatureWhitenTorch:
    ...
    def fit(self, dataloader):
        ...
        for batch in dataloader:
            ...
            # batch is [B, C, H, W], already from ToTensor() in [0, 1]
            batch = batch.reshape(batch.shape[0], -1)

            # same as subtract_mean_and_normalize_h_numpy
    ...
    def transform(self, x):
        ...
        return x.reshape(original_shape if len(original_shape) == 4 else (x.shape[0], *original_shape))
...
def precompute_zca_dataset(dataset_obj, zca, batch_size=256, num_workers=0):
    loader = DataLoader(
        dataset_obj,
...
def get_data(dataset='cifar10', root='datasets', batch_size=32, num_workers=0, whiten_lvl=None):
    ...
    reproduction_mode = None#"New"#"New" Set to "New" for Unit-Scale Preprocessing
    ...
    if whiten_lvl is not None:
        ...
        if reproduction_mode=="New":
            ...
            zca = FeatureWhitenTorch(eps=1e-5)
            ...
            zca.fit(temp_loader)#, dataset=dataset)#, dataset="stl10")
            ...
            ###pre = torch.stack([PerSampleRMSNormalize()(x) for x in original_batch])
            ###whitened_batch = zca.transform(pre)
            ###whitened_batch = torch.stack([UnitScale()(x) for x in whitened_batch])
            ...
            tst_set.transform = basic_transform

        else:
            print("Mondragons preprocessing pipeline")
            ...
            trn_set.transform = base_transform
            tst_set.transform = base_transform
          ...
          #  
            ...
            trn_set = precompute_zca_dataset(
                trn_set,
                zca,
                batch_size=256,
                num_workers=0,
            ...
            tst_set = precompute_zca_dataset(
                tst_set,
                zca,
                batch_size=256,
                num_workers=0,
            ...
            train_aug = crop_flip(img_size) if dataset in ["cifar10", "stl10"] else None

            trn_set = AugmentedTensorDataset(
                trn_set,
                transform=train_aug,
            )

            tst_set = AugmentedTensorDataset(
                tst_set,
                transform=None,
            )
