...
from visualisation.receptive_fields import visualize_filters, visualize_filters_1x3_publication
...
if __name__ == "__main__":
    ...
    hebb_param = get_hebbian_config(version="hpca_config")#get_hebbian_config(version="journe_like_config")
    device = torch.device('cuda:0')
    print(device)
    model = Net_Hebbian(hebb_params=hebb_param, version="mnist") #choose stl_net for STL10 experiments
    model.to(device)
    ...
    hebb_params = [
      ...
      #  {'params': model.conv4.parameters(), 'lr': 0.1},  # COMMENT THIS LINE IN for deeper STL10 net
    ]
    ...
    if mode == "soft":
        ...
        ZCA_White = 1e-3
    trn_set, tst_set, zca = get_data(dataset='mnist', root='../datasets', batch_size=64,
    ...
    visualize_filters_1x3_publication(
        model,
        layers=None,
        layer_titles=("conv1", "conv2", "conv3"),
        num_filters=25,
        input_channels=3,
        step_size=0.001,
        iterations=500,
